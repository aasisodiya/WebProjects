...
function generateCalendar(yyyyMM) {
    ...
    let month = selectedTime.toLocaleString("default", {
        month: "long"
    });
    ...
    let template =
        "<div class='classname'><div>replacement</div><span><!-- emoji --></span></div>";
    let titleRow = "";
    weekdayInitials.forEach((weekday) => {
        titleRow += template
            .replace("replacement", weekday)
            .replace("classname", "weekdayInitials");
    });
    ...
    for (let index = 0; index < activedays; index++) {
        monthDays += template
            .replace("replacement", index + 1)
            .replace("classname", "day");
    }
    for (let index = activedays; index < numberOfDays; index++) {
        monthDays += template
            .replace("replacement", index + 1)
            .replace("classname", "disabledday");
    }
